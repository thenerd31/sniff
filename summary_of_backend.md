...
- **Score >= 50 + product detected** -> "This is a scam. Finding legitimate alternatives..." -> runs full price comparison -> green price cards fly in after red investigation cards
...
```
src/app/api/investigate/route.ts    -- Unified agent route (investigation + price comparison)
src/app/api/deepen/route.ts         -- Multi-turn deepening
src/app/api/compare/route.ts        -- Standalone price comparison
src/app/api/investigate-demo/route.ts -- Pre-cached demo fallback

src/lib/stream.ts                   -- SSE response helper
src/lib/scoring.ts                  -- Veto threat scoring system
src/lib/known-domains.ts            -- High-authority domain allowlist (~150 domains)
src/lib/investigationStore.ts       -- In-memory investigation state (for multi-turn)
src/lib/hooks/useInvestigation.ts   -- Frontend SSE consumer hook

src/lib/tools/
  whois.ts                          -- WHOIS domain lookup
  ssl.ts                            -- SSL certificate analysis
  safe-browsing.ts                  -- Google Safe Browsing API
  brand-impersonation.ts            -- LLM-based brand detection
  reddit.ts                         -- Reddit scam report search
  scraper.ts                        -- Page red flag scanner
  scamadviser.ts                    -- ScamAdviser trust score
  webSearch.ts                      -- Perplexity web search
  brightdata.ts                     -- Bright Data structured scraping
  browserbase.ts                    -- Browserbase AI browser (Stagehand)
  priceSearch.ts                    -- Perplexity retailer link finder + price fallback

src/types/index.ts                  -- Shared type definitions (EvidenceCard, SSEEvent, etc.)
```
